...
**Why you need saved conversations and logs too, not just memory.md** — A session's own conversation isn't durable: once it ends, or the context window fills and gets summarized, anything that wasn't written to a file is gone unless you deliberately resume that exact session. Three different layers cover three different needs: a **saved conversation** (full transcript + summary — see Useful Skills below) is for picking a specific interrupted task back up or finishing something later, with the full detail of exactly what was said; **`Logs/Status.md`** is a running, quickly-skimmable record so an agent (or you) can tell what's been going on lately without rereading a full transcript; **`memory.md`** holds only the distilled, durable facts that should shape behavior going forward, not a play-by-play. See [00-Skills-Catalog](../00-Skills-Catalog/Guide.md) for the actual skills that implement all three (`save-conversation`, `logs`, `load-logs`).
...
2. Give the agent a short name/handle (e.g. "Steve") — pick something you'll actually type or say. This becomes the wrapper command name (step 8) and the key any chat/remote-control layer uses to find the right folder or file.
   ...
   If you're building more than one agent that a single external trigger source (a phone automation tool, a shared picker UI, another script) needs to address by name, pick one uniform transform rule up front — e.g. "lowercase, strip all spaces and hyphens" — and make every agent's wrapper/param name conform to it, rather than hand-picking a slug per agent. A per-agent slug (`the-man`, `projectmanager` — inconsistent hyphenation) forces the caller to maintain a lookup table just to get the name right. A uniform rule means the caller can derive the correct name with one deterministic formatting step, with no per-agent exceptions and no table to keep in sync as agents are added.
...
- **Renaming an agent's handle later is safe *only* if you confirm what actually calls the wrapper name.** If a webhook or workflow builds the wrapper/script name dynamically from an input parameter (e.g. `start-<agent>`), renaming the script is a one-line fix with zero orchestrator changes. If instead the name is hardcoded somewhere (a workflow's per-agent branch, a static mapping table), a rename requires updating that config too — check which situation you're in before renaming, and audit every doc/reference file that names the old handle (status logs, other agents' memory files, shared change logs, skill files that use the name in prose examples) so nothing goes stale. Treat a rename as a small migration, not just a file move.
- **Redacting-with-a-preview is fundamentally unsafe once a file is already known to contain a real secret — no regex anticipates every format.** Markdown `**bold**` wrapping, backtick-quoted values, an unexpected token prefix — there's always a gap. If a file is confirmed to hold a real secret, don't preview it with "context first, redact after"; replace by line number blind, and verify success via a boolean/hash check, never by looking at the result.
- **A "Creator" agent (one whose job is building other agents) should never edit blueprint content itself, even when it finds a real gap.** Keep the same discipline as any other agent that touches this documentation: write a dated handoff report for a human/maintainer to review and fold in, rather than modifying blueprint files directly. Keeps blueprint changes deliberate and reviewed instead of silently drifting.
- **The opposite mistake is just as real: a fuzzy "mask anything that looks risky" heuristic destroys real, benign content.** One case: the word "Manager" (as in "Password Manager agent") got masked because a heuristic treated any short word following "password" as a possible secret. Only ever redact by an exact, already-confirmed value (string match) or a specific line already verified some other way (hash/live-test) — never a "this looks suspicious" guess.
- **A scratch/dump note (an "Untitled.md"-style catch-all) is a common and easily-missed leak vector.** Real secrets get pasted there bare, with no "password:"/"token:" label anywhere nearby — a keyword-based scan has no chance of finding them. Any project involving credential cleanup should include a manual read-through of obvious catch-all notes, not just an automated pattern scan.
- **Once you know one secret's exact value, search the whole knowledge base for that exact string, not just the "main" location you already fixed.** The same value tends to resurface unlabeled in transcripts, summaries, and other notes well beyond wherever it was first found.
- **The root-cause fix for plaintext secrets is upstream, not another cleanup pass.** A one-time audit doesn't stay clean — build the "don't write secrets in plaintext" discipline directly into whatever creates the agent's logs/transcripts/memory going forward (see the save-conversation/close-out skills above), rather than relying on periodic re-sweeps.
...
**Gotcha — the webhook can lie about success if the launcher backgrounds itself.** If the SSH command that launches the remote-control terminal backgrounds itself (`nohup some-script > log 2>&1 </dev/null &`), the SSH node — and therefore the webhook — reports success the instant the shell accepts the command, before it's known whether the backgrounded command actually did anything. A real case: the trigger payload had a slightly wrong agent name; the launcher script for that name didn't exist, the backgrounded `nohup` call failed silently, and the webhook still returned 200 "started" — no error surfaced anywhere, not even in the intended log file, because the failure happened before the log redirect took effect. Fix: don't background blind — gate the launch with a synchronous check first, so the SSH node's own stdout (available *before* backgrounding returns) carries a real signal: `if [ -x /path/to/launcher-for-<name> ]; then nohup launcher-for-<name> ... & echo LAUNCHED; else echo NOTFOUND; fi`, then branch the webhook's response on that stdout (e.g. an n8n IF node) rather than assuming "SSH exited 0" means the thing actually happened.

**Gotcha — trim and validate whatever's triggering the webhook, not just what your workflow tool receives.** A phone automation tool (Tasker, Shortcuts) posting the trigger is more likely to introduce stray leading/trailing whitespace in a field than a hand-typed API call — enough to break a strict-equality match downstream. Trim/validate every field from an external trigger source in the workflow itself, even one that "should never" send anything malformed. Format-only validation (e.g. a regex checking the name looks well-formed) isn't the same as checking the target actually exists — a well-formed-but-wrong name should resolve to a clear 404, not a false-positive "started."

**Gotcha — a naming inconsistency across agents pushes translation logic onto whatever's calling the webhook.** If one agent's param is `the-man` and another's is `projectmanager`, a phone automation tool (or any external caller) either needs a hardcoded per-agent mapping (fragile, easy to forget to update when an agent is added) or has to special-case the hyphenated ones. Standardizing every agent's param to the same transform (e.g. lowercase, no separators) up front means the caller can use one generic formatting step — a real win if the caller is something like Tasker's Variable Search Replace action rather than a general-purpose scripting environment.

**Gotcha — a workflow tool's live-edit hot-reload isn't infinitely stackable in one day.** Editing an active n8n workflow via its API (`PUT`) rather than a full container restart avoids a webhook re-registration blip on a single change, and that's still the right call for one edit. But multiple same-day live edits to the *same* workflow, with no restart in between, can cause intermittent multi-second response-latency stalls on that workflow's webhook (engine-level queuing, not a per-node slowdown — individual node compute times stay normal even during a slow run) — a single container restart clears it completely. Practical rule: after 2-3 same-day live edits to the same workflow, restart the container anyway rather than assuming the hot-reload path stays clean indefinitely. If a latency regression like this gets reported, compare against the workflow's own execution history (per-node timing) first before assuming a code-level cause.

## Bonus (Optional): Repo Update Checker

Not required — this is for an agent built from these blueprints to notify you when the blueprints repo itself changes upstream, so you don't have to remember to check back manually.

1. Set up a scheduled job — same mechanism as [Blueprint #5](../05-Scheduled-Self-Reporting/Guide.md)'s self-reporting jobs (cron, systemd timer, or n8n) — on a slow cadence. Monthly is plenty; this is a docs repo, not a live service.
2. The job calls the GitHub API, unauthenticated: `GET https://api.github.com/repos/Mikeaallen90/homelab-ai-blueprints/commits?sha=main&per_page=1` and reads the top commit's SHA.
3. Compare that SHA to one stored locally from the last check (a small file, one line). First run: just store it — nothing to compare against yet.
4. If it differs: notify via whatever channel the agent already has (Telegram from the Bonus section above, or an appended note to a log/Obsidian note if no chat channel exists), then update the stored SHA.
5. Point the notification at the repo's CHANGELOG (once one exists) or its `/commits` page, so it says *what* changed, not just that something did.

**Gotcha**: GitHub's unauthenticated API is capped at 60 requests/hour per IP — a non-issue at monthly cadence, but worth knowing if you extend this to watch several repos from the same machine on a tighter interval.

**Adaptation note**: nothing here is specific to this particular repo — the same script/skill works for watching any GitHub repo you care about, just swap the owner/repo in the API URL.
